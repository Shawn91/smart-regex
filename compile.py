...
def compile_tokens_to_nested_tokens(tokens: List[Token], nested_tokens: List = None) -> List[List[Term]]:
    """
    ...
    Example:
        ...
        ['a', '(', 'c', 'v', ')', '+']: returns [['a', ['c', 'v'], '+']].
    Tests:
    >>> compile_tokens_to_nested_tokens(convert_pat_str_to_tokens('a\+b'))
    [TEXT:a, TEXT:+, TEXT:b]
    >>> compile_tokens_to_nested_tokens(convert_pat_str_to_tokens('a(b)+c'))
    [TEXT:a, [TEXT:b], PLUS:+, TEXT:c]
    >>> compile_tokens_to_nested_tokens(convert_pat_str_to_tokens('(b)c'))
    [[TEXT:b], TEXT:c]
    >>> compile_tokens_to_nested_tokens(convert_pat_str_to_tokens('a(b)'))
    [TEXT:a, [TEXT:b]]
    >>> compile_tokens_to_nested_tokens(convert_pat_str_to_tokens('a(b(c))'))
    [TEXT:a, [TEXT:b, [TEXT:c]]]
    >>> compile_tokens_to_nested_tokens(convert_pat_str_to_tokens('a(b(c)d)e'))
    ...
    while token_idx < len(tokens):
        ...
        if cur_token.is_escape:
            ...
            compile_tokens_to_nested_tokens(tokens[token_idx + 1:], nested_tokens=nested_tokens[-1])
            ...
            nested_tokens.append([])
            compile_tokens_to_nested_tokens(tokens[token_idx + 2:], nested_tokens=nested_tokens[-1])
            count = count_list_elements(nested_tokens[-1])
    ...
    while token_idx < len(nested_tokens):
        ...
        if isinstance(cur_token, Token):
            if cur_token.is_normal:
                ...
                token_idx += 1
        else: # cur_token is actually a list
            pass
...
if __name__ == '__main__':
    import doctest
    doctest.testmod()
    tokens = convert_pat_str_to_tokens('ab')
    nested_tokens = compile_tokens_to_nested_tokens(tokens)
    terms = compile_nested_tokens_to_terms(nested_tokens)
    print(terms)
